[2026-07-09] col4+col5 横版回退到7月1日纯LLM生成(frameset印刷底板/结构化模板/代码填充全部停用, col3零改动):
背景: 用户实测本会话(7/9)加的 frameset印刷底板架构+结构化模板+双向fit/对比度校正, 产出质量很差——(a)封面/目录标题、副标题、脚注被LLM强行扩写成整句/整段(我上一轮加的_FRAMESET_SYS"每槽填够60%-100%"提示词逼的), 溢出后代码缩成9px小字→"和WORD差不多"; (b)字号不遵循VI手册(我上一轮代码grow把字号放大到提取值1.8×, 违背"字号锁死=VI提取值"); (c)排版布局全乱。用户决定: col4和col5都回退到7月1日以前的纯大模型生成HTML(无框架/印刷底板/代码填充/结构化模板), col3完全不动。
真相核实(git show eddb446:ppt_service.py): 7月1日 col4/col5 走同一条纯LLM路径, _gen_one里唯一分支是is_a4(区分col3竖版vs col4/col5横版), 没有frameset/_code_filled/结构化模板/column_id=="col4"——全是7/4(40f3698结构化模板)和7/9(af0938b frameset)新加的。
方案(外科手术, 非整体回退文件——因col3的A4 pipeline/TOC编辑器/封面flex等7月1日后的独立改进也在同一文件, 整体回退会误伤col3): 只删_gen_one里4个横版(not is_a4)代码填充"进入分支", 让横版页自然落回本来就存在的LLM生成路径(=7月1日行为); col3走is_a4分支完全绕过被删块, 零影响。删除内容(ppt_service.py, 纯删除98行0新增): (1)frameset catalog预加载块(保留_done_lock/_done_count, LLM路径仍用); (2)`if _frameset and not is_a4`的frameset pick+fill分支; (3)`if stype in STRUCTURAL_PAGE_TYPES and not is_a4`的结构化模板分支; (4)`if column_id=="col4" and not is_a4`的col4专属内容页代码填充。保留(死代码不删, 降风险, 运行时不触达): _frameset_pick_and_fill/_select_framework_template/_fill_slide_template/STRUCTURAL_PAGE_TYPES/_fit_code_filled_slides定义、frameset_service.py、framesets/*.md、business.json。_fit_code_filled_slides调用保留(只处理_code_filled页, 回退后横版不再产此标记、col3从无→自动空操作)。
验证(走生产函数_stage2_html_per_slide逐页出数): col4 11页全纯LLM(框架=0 代码填充=0) PASS; col5 16页全纯LLM PASS; col3 11页正常生成(cover/toc/table/chart/diagram/flowchart/closing各类型无报错)零回归。git diff确认98行纯删除、含is_a4的删除行全是`not is_a4`(横版进入条件)、无一行碰col3的is_a4逻辑; ast语法OK。后端重启PID29828(port8766 /api/settings 200)。
暂不做: 删frameset死代码(本轮只断调用不删定义); col4质量提升(框架方向)本轮全部作废回7月1日基线。
