    ...
    with _batch_lock:
        for batch_id, job in list(_active_batches.items()):
            ...
            for item in job.items:
                if item.get("project_id") == project_id and item.get("status") not in ("completed", "failed", "skipped"):
                    return {
                        "batch_id": batch_id,
    ...
    try:
        ...
        try:
            ...
            for br in batch_rows:
                row = db.execute(
                    "SELECT project_id, status FROM batch_job_items WHERE batch_id=? AND project_id=? AND status NOT IN ('completed','failed','skipped')",
                    (br[0], project_id),
                ).fetchone()
                if row:
                    return {
...
def get_active_batches(workspace_id: str = "") -> list[dict]:
    """Return list of active (pending/running/stopped) batches. Checks memory first, then DB.
    When workspace_id is provided, only returns batches for that workspace."""
    result = []
    with _batch_lock:
