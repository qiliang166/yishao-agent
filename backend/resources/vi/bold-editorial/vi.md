...
1. **结构化的层级** — 通过严格的 Inter 字体字号体系、字重和间距，建立一目了然的信息架构。标题 600/700 粗体、正文 400 常规，层级分明，不依赖颜色区分。
...
| 角色 | 字体栈 |
|------|--------|
| 标题 | `Playfair Display, Georgia, 'PingFang SC', 'Microsoft YaHei', serif` |
| 正文 | `Source Sans 3, Arial, 'PingFang SC', 'Microsoft YaHei', sans-serif` |
...
### Big Number（大数字）
- 居中 48px Inter 加粗 + 单位 16px + 标签 13px opacity 0.5
- 颜色使用 chart_color
...
4. **页码统一** — 所有页面右下角统一放置页码（12px Inter, opacity 0.4）
...
11. 全页字体统一使用 Inter 家族（typography token）
12. 卡片圆角统一 6px
