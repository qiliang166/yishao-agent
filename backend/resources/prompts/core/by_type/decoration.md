...
- 装饰克制：可用半透明几何元素点缀角落，半径不超过画布短边的 25%，opacity 0.03-0.08
- 网格纹理 pattern 可选（40×40px，0.5px stroke，opacity 0.02-0.04）
- 装饰元素数量适中，以内容为主角，装饰不抢夺视觉注意力
...
- 顶部 accent 色条（固定）
- 标题短线（固定）
- 装饰元素克制：1-2 个半透明几何点缀足够，opacity 0.03-0.06，角落放置
- 可选网格纹理（仅暗色模式）
- 装饰不应覆盖或干扰卡片内容区域
