...
def _lock_hidden_project_ids(db, user: dict, booklet: dict) -> dict:
    """返回 {project_id: point_cost_deci}：当前查看者应隐藏正文的付费明细。

    管理员与册子创建者豁免；普通会员对 preview_requires_unlock=1 且未解锁的明细隐藏正文。
    """
    ...
    marks = ",".join("?" * len(pids))
    rows = db.execute(
        f"SELECT id, point_cost_deci FROM projects WHERE id IN ({marks}) AND preview_requires_unlock=1",
        tuple(pids),
    ).fetchall()
...
def get_booklet(booklet_id: str, request: Request):
    ...
    try:
        ...
        if hidden:
            for ch in booklet["chapters"]:
                if (ch.get("project_id") or "") in hidden:
                    ch["locked"] = True
                    ch["content"] = ""
                    ch["content_html"] = ""
        return booklet
