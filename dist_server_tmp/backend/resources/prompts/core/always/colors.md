...
|---------|---------|------|
| 正文文字（浅色底） | `color: {{text}}` | — |
| 正文文字（**所在元素或祖先 background 为深色，亮度 ≤ 128**） | `color: #ffffff` | 通用例外，覆盖 text 默认；见下方"对比度铁律" |
...
### 对比度铁律（元素级 — 唯一的 text 默认例外）

**任何元素的 `background` 为深色（`primary`/`secondary`/深 `chart` 色/hero 渐变，即背景亮度 ≤ 128）时，其内部所有文字必须用白色 `#ffffff`，覆盖 `{{text}}` 的全页正文默认。**

- 这条是"正文用 `{{text}}`"规则的**唯一例外**，凌驾于全页正文默认之上。
- 不局限于封面/hero 卡/表头等列举场景——**任意**深色底元素（含你即兴创建的深蓝渐变卡、深色高亮块）都适用。
- 判断依据是"文字所在元素或其任一祖先的实际背景色深浅"，不是"整页背景深浅"。深色底写 `#ffffff` 完全合规（`#ffffff` 是系统唯一合法硬编码 hex）。
