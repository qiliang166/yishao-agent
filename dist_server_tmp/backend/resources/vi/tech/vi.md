...
1. **清晰的层级** — 通过严格的字体大小、字重和间距，建立一目了然的信息架构。标题 Space Grotesk 粗体、正文 Inter 常规，层级分明。
...
| 角色 | 字体栈 |
|------|--------|
| 标题 | `Space Grotesk, Inter, 'PingFang SC', 'Microsoft YaHei', sans-serif` |
...
### Big Number（大数字）
- 居中 52px Space Grotesk 加粗 + 单位 18px + 标签 14px opacity 0.6
- 颜色使用 chart_color
...
14. 字体来自 typography token（Space Grotesk + Inter）
