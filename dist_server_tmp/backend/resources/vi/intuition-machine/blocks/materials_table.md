...
```html
<div style="width:794px;height:1123px;display:flex;flex-direction:column;background:var(--background);font-family:Space Grotesk, Space Grotesk, Inter, 'PingFang SC', 'Microsoft YaHei', sans-serif;position:relative;overflow:hidden;">
  ...
  <div style="flex:1;position:relative;overflow:hidden;">
    ...
    <table style="width:100%;border-collapse:collapse;font-size:12px;font-family:Space Grotesk, Space Grotesk, Inter, 'PingFang SC', 'Microsoft YaHei', sans-serif;">
      <colgroup>
...
| `{{TABLE_ROWS}}` | 所有 `<tr>` 行拼接，每行8个 `<td>`，照行模板格式 |
| `{{SEQ}}` | 序号（从1开始） |
| `{{CATEGORY}}` | 食材分类（干货/海鲜/禽类/调辅料/香料/调味品/汤水/淀粉） |
| `{{NAME}}` | 食材名称 |
| `{{BRAND}}` | 品牌或产地（原文未提及则填"—"） |
| `{{PROCESS_NOTE}}` | 加工说明（如"干蒸后50℃浸泡8h"） |
| `{{PROCESS_REQ}}` | 加工要求（如"胶质不流失"） |
| `{{WEIGHT}}` | 数值（如"100"） |
| `{{UNIT}}` | 单位（克/毫升/个/片/颗） |
