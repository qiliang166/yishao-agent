...
6. **填充铁律**：1280×720 画布的内容区 (y:60-660, x:60-1220) 必须被完全填满。任何 >80px 的连续空白区域 = 设计失败。无内容可填时必须用 SVG 装饰图形、几何图案或 `{{image:}}` 占位符填补。**代码会自动检测空白区域，违规将强制重试。**

上述六条铁律在后续章节中有详细说明和 HTML 示例。铁律优先于所有其他设计考量——即使它"看起来更好看"，违反就是错的。
