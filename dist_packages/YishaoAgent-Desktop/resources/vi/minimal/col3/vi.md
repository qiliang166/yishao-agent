...
| 变量 | 用途 |
|------|------|
| `var(--primary)` | 标题文字色、标签列文字色 |
| `var(--accent)` | 标题底线、链接强调 |
| `var(--background)` | 内部页面底色 |
| `var(--text)` | 正文文字 |
| `var(--card_bg)` | 封面背景、标签列背景 |
...
- 删除模板中已有的元素
- 硬编码 hex 色值（除 `#ffffff`）
- 使用 linear-gradient 作为封面背景（必须使用纯色 `var(--card_bg)`）
